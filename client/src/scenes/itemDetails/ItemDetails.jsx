...
const ItemDetails = () => {
  ...
  async function getItem() {
    const itemp = await fetch(
      `http://localhost:1337/api/items/${itemId}?populate=image`,
      {
        method: "GET",
      }
    );
    const itemJson = await itemp.json();
  ...
  async function getItems() {
    const itemsp = await fetch(
      `http://localhost:1337/api/items?populate=image`,
      {
        method: "GET",
      }
    );
    const itemsJson = await itemsp.json();
  ...
  useEffect(() => {
    console.log(item?.data?.attributes?.longDescription[0].children[0].text)
  }, [item])
